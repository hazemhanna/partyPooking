...
class Helper {
   ...
   class func getAPIToken() -> String? {
    ...
    class func saveAPI( token: String,user_id: Int,email: String, name: String,phone :String) {
        let def = UserDefaults.standard
        def.set(token, forKey: "token")
        def.set(user_id, forKey: "id")
   ...
   class func getPartyPrice() -> [[String : Any]]? {
    ...
    class func saveSilent(isOn: Bool) {
        let def = UserDefaults.standard
        def.set(isOn, forKey: "isOn")
        def.synchronize()
    }
    
   class func getSilent() -> Bool? {
      let def = UserDefaults.standard
      return def.object(forKey: "isOn") as? Bool
   }
}
